...
# функциив map() использовать lambda.
...
# (которые читатются одинаково в обе стороны)

print(
    list(
        filter(
            lambda word: word == word[::-1] ,
            ('казак', 'армян', 'доход', 'лиилол'),
        )
    )
)
...
# 5) Сделать функцию которая которая на вход принимает строку, анализирует её исключительно методом .isdigit() без
